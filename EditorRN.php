...
{
  public static $VERSAO_CK = '15032016b';
  ...
  protected function montarSimplesConectado(EditorDTO $objEditorDTO)
  {
    ...
    if($objOrgaoDTO->getStrStaCorretorOrtografico()==OrgaoRN::$TCO_NATIVO_NAVEGADOR){
        ...
        foreach ($arrObjVersaoSecaoDocumentoDTO as $objVersaoSecaoDocumentoDTO) {
          if ($objSecaoDocumentoDTO->getNumIdSecaoDocumento()==$objVersaoSecaoDocumentoDTO->getNumIdSecaoDocumento()) {
            ...
            if ($strConteudo!=$objVersaoSecaoDocumentoDTO->getStrConteudo()) {
              ...
              //atualiza acentuação para código HTML
              $strConteudo2 = self::converterHTML($strConteudo);
              
              if ($strConteudo2!=$objVersaoSecaoDocumentoDTO->getStrConteudo()) {
                $bolSecaoAlterada = true;
                $strConteudo=$strConteudo2;
              }
            }
            break;
          }
  ...
  private function obterParametros(EditorDTO $parObjEditorDTO,$parStrTag){
    ...
    try {
      ...
      if ($bolUnidade) {
        ...
        $strSinEnderecoAssociado = $objContatoDTO->getStrSinEnderecoAssociado();

        if ($strSinEnderecoAssociado=='N') {
          self::$arrTags['endereco_unidade'] = $objContatoDTO->getStrEndereco();
        }else{
          self::$arrTags['endereco_unidade'] = $objContatoDTO->getStrEnderecoContatoAssociado();
        }
        ...
        if ($strSinEnderecoAssociado=='N') {
          ...
          self::$arrTags['cep_unidade'] = 'CEP ' . $objContatoDTO->getStrCepContatoAssociado();
        }

        if ($strSinEnderecoAssociado=='N') {
          self::$arrTags['sigla_uf_unidade'] = $objContatoDTO->getStrSiglaUf();
        }else{
          self::$arrTags['sigla_uf_unidade'] = $objContatoDTO->getStrSiglaUfContatoAssociado();
        }
        ...
        if ($strSinEnderecoAssociado=='N') {
          ...
          if (!InfraString::isBolVazia($objContatoDTO->getStrBairroContatoAssociado())) {
            $strTag = ' - Bairro ' . $objContatoDTO->getStrBairroContatoAssociado();
          }
        }
        ...
        if ($strSinEnderecoAssociado=='N') {
          ...
          if ($objContatoDTO->getStrNomeCidadeContatoAssociado() != '') {
            self::$arrTags['cidade_unidade'] = $objContatoDTO->getStrNomeCidadeContatoAssociado();
          }
        }
        ...
        if ($strSinEnderecoAssociado=='N') {
          ...
          if (!InfraString::isBolVazia($objContatoDTO->getStrComplementoContatoAssociado())) {
            $strTag .= $objContatoDTO->getStrComplementoContatoAssociado();
          }
        }
      ...
      if ($parObjEditorDTO->getDblIdDocumento()!=null && $bolParticipante) {
        ...
        if (count($arrObjParticipanteDTO)){
          ...
          $objContatoRN->removerDadosPrivados($arrObjContatoDTO);
...
//    LogSEI::getInstance()->gravar('M3='.$matches[3].'  - '.SessaoSEI::getInstance()->getNumIdSistema());
  ...
  private function montarTagsContato(ContatoDTO $objContatoDTO, $strTipo){
    ...
    if ($strSinEnderecoAssociado == 'N') {
      ...
      $strTag = $objContatoDTO->getStrEnderecoContatoAssociado();
    }
    ...
    if ($strSinEnderecoAssociado=='N') {
      ...
      $strTag = $objContatoDTO->getStrBairroContatoAssociado();
    }
    ...
    if ($strSinEnderecoAssociado=='N') {
      ...
      $strTag = $objContatoDTO->getStrNomeCidadeContatoAssociado();
    }
    ...
    if ($strSinEnderecoAssociado=='N') {
      ...
      $strTag = $objContatoDTO->getStrCepContatoAssociado();
    }
    ...
    if ($strSinEnderecoAssociado=='N') {
      ...
      if ($objContatoDTO->getStrSiglaUfContatoAssociado()!='') {
        $strTag = ' - ' . $objContatoDTO->getStrSiglaUfContatoAssociado();
      }
    }
    ...
    if ($strSinEnderecoAssociado=='N') {
      ...
      $strTag = $objContatoDTO->getStrNomePaisContatoAssociado();
    }
